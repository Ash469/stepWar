...
//     _selectedBot = _botService.getNextBot(userWins);
...
//           _buildInfoRow('👑 K.O. Victory', 'Your Score + 3000 Coins'),
//           const SizedBox(height: 12),
//           _buildInfoRow('🏆 Win', 'Your Score + 1000 Coins'),
//           const SizedBox(height: 12),
//           _buildInfoRow('🤝 Draw', 'Your Score '),
//         ],
//       ),
//     );
//   }
...
class BotSelectionScreen extends StatefulWidget {
  final UserModel user;
  final BotType botType; // Accept the specific bot from home
...
class _BotSelectionScreenState extends State<BotSelectionScreen> {
  ...
  void _initiateBattle() async {
    // 1. Brief delay for visual polish
    await Future.delayed(const Duration(seconds: 1));
    if (!mounted) return;

    setState(() {
      _statusText = "Creating Arena...";
    });

    try {
      // 2. Use the specific bot ID passed from the previous screen
      final String selectedBotId = _botService.getBotId(widget.botType);
  ...
  @override
  Widget build(BuildContext context) {
    final botName = _botService.getBotNameFromId(_botService.getBotId(widget.botType));
    final botImage = _botService.getBotImagePath(widget.botType);
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            // Display the specific bot icon/image
            Container(
              ...
              child: Image.asset(botImage, height: 120, width: 120),
            ),
