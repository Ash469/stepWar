...
class DatabaseHelper {
  ...
  Future<String> createUser(String nickname) async {
    ...
    final user = GameUser(
      id: userId,
      nickname: nickname,
      totalSteps: 0, // Start with 0 steps - points will be earned through actual walking
      attackPoints: 0, // Start with 0 attack points - will be earned through steps
      shieldPoints: 0, // Start with 0 shield points - will be earned through steps
      lastAttackReset: now,
      createdAt: now,
      updatedAt: now,
    );
  ...
  /// Give user some testing points for immediate gameplay (debug/testing only)
  Future<void> giveUserTestingPoints(String userId, {int attackPoints = 50, int shieldPoints = 50}) async {
    final user = await getUser(userId);
    if (user == null) return;
    
    final updatedUser = user.copyWith(
      attackPoints: user.attackPoints + attackPoints,
      shieldPoints: user.shieldPoints + shieldPoints,
    );
  ...
  Future<List<Territory>> getAttackableTerritories(String userId) async {
    ...
    final maps = await db.query(
      'territories',
      where: '''owner_id != ? AND status = 'peaceful' AND 
